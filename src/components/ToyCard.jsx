...
const ToyCard = ({ toy }) => {
  ...
  const handleViewMore = () => {
    if (!user) {
      ...
      navigate("/login", { state: { from: `/toy/${toy.toyId}` } });
    } else {
      // If logged in, go to toy details page
    ...
    <div className="bg-white rounded-lg shadow-lg overflow-hidden transform hover:scale-105 transition-transform duration-300">
      ...
      <div className="p-6">
        ...
        <p className="text-gray-600 h-12 overflow-hidden text-ellipsis">
          {toy.description.substring(0, 60)}...
        </p>
        <div className="mt-6">
          <button
            onClick={handleViewMore}
            className="w-full bg-pink-500 text-white font-semibold py-2 px-4 rounded-md hover:bg-pink-600 transition duration-300"
          >
            View More
          </button>
        </div>
